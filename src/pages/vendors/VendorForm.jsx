...
import {
  ...
  AlertCircle, RefreshCw, Printer, ChevronLeft, Truck,
} from "lucide-react";
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {form.code ? form.code.slice(0, 2).toUpperCase() : (form.name ? form.name.slice(0, 2).toUpperCase() : <Truck size={22} />)}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">{isNew ? "New Vendor" : (form.name || "Vendor")}</h1>
              <p className="text-sm text-white/70 mt-0.5 flex items-center gap-2">
                {form.code ? <span className="font-mono">{form.code}</span> : <span>Add a new vendor to the master</span>}
                {(mode === "new" || mode === "edit") && (
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-2">
                  ...
                  {(isReadOnly && form.shipFromSameAsVendor) && (
                    <p className="text-xs text-brand-500 font-medium">✓ Ship From — same as Corporate Address</p>
                  )}
                  <AddressBlock
                ...
                <div className="space-y-2">
                  ...
                  {(isReadOnly && form.billToSameAsShipFrom) && (
                    <p className="text-xs text-brand-500 font-medium">✓ Bill To — same as Ship From</p>
                  )}
                  <AddressBlock
