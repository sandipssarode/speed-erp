...
import {
  Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, ClipboardList,
} from "lucide-react";
...
function validate(form, allRecords, editingId) {
  ...
  if (!form.typeName.trim()) e.typeName = "Work Order Type Name is required.";
  return e;
}
...
export default function WorkOrderTypeForm() {
  ...
  useEffect(() => {
    api.get("/api/work-order-types").then(list => {
      setAllRecords(list);
      if (!isNew && id) {
  ...
  const handleSave = async () => {
    ...
    try {
      let saved;
      if (isNew) {
        saved = await api.post("/api/work-order-types", {
          ...form,
        ...
        saved = await api.put(`/api/work-order-types/${id}`, {
          ...form,
          updatedAt: now, updatedBy: userName,
          changelog: [...(form.changelog || []), entry],
        });
      }
      setForm(saved);
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {!isNew && form.typeName
                ? form.typeName.slice(0, 2).toUpperCase()
                : <ClipboardList size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">
                {isNew ? "New Work Order Type" : (form.typeName || "Work Order Type")}
              </h1>
              <p className="text-sm text-white/70 mt-0.5">
                {isNew ? "Add a new work order classification type" : <span className="font-mono">{form.typeId}</span>}
              </p>
            </div>
                  ...
                  <tbody>
                    {form.changelog.map((c, i) => (
                      <tr key={i} className="border-t border-gray-100">
                        ...
                        <td className="py-1.5">
                          <span className={`px-1.5 py-0.5 rounded text-[11px] ${c.action === "Created" ? "bg-green-50 text-green-600" : "bg-brand-50 text-brand-600"}`}>
                            {c.action}
                          </span>
                        </td>
                      </tr>
          ...
          <div className="px-6 py-7 space-y-6">
            <Row label="Type ID" required error={errors.typeId} help={isNew ? "Auto-suggested — you may edit before saving; cannot be changed later." : undefined}>
              <input
                ...
                placeholder="e.g. WOT-001"
                maxLength={20}
                className={inputCls(isReadOnly || !isNew, errors.typeId)}
              />
            </Row>

            <Row label="Work Order Type Name" required error={errors.typeName}>
              <input
                value={form.typeName}
                onChange={e => setField("typeName", e.target.value)}
                disabled={isReadOnly}
                placeholder="e.g. Manufacturing Order"
                className={inputCls(isReadOnly, errors.typeName)}
              />
            </Row>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button
                onClick={handleSave}
                className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"
              >
                <Save size={15} /> Save Work Order Type
              </button>
              <button
                onClick={handleDiscard}
                className="text-sm px-5 py-2.5 border border-gray-300 text-gray-700 hover:bg-white rounded-xl font-semibold transition-colors"
              >
                Cancel
              </button>
            </div>
