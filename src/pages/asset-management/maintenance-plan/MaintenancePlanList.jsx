...
const STATUS_DOT = { Active: "bg-green-500", Inactive: "bg-gray-400" };
...
export default function MaintenancePlanList() {
  ...
  const filtered = records
    ...
    .sort((a, b) => {
      const r = (a.nextDueDate || "").localeCompare(b.nextDueDate || "");
  ...
  const handleDelete = async (id, planName) => {
    if (!window.confirm(`Delete maintenance plan "${planName}"? This cannot be undone.`)) return;
    try {
  ...
  return (
    <Layout>
      <div className="w-full space-y-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">Maintenance Plan</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Preventive maintenance schedules with auto job creation
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[220px] max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search by Plan ID, name or asset…"
              value={search}
          ...
          <button
            onClick={() => navigate("/asset-management/maintenance-plan/new")}
                ...
                <tr className="bg-gradient-to-r from-brand-800 to-brand-600">
                  ...
                  <th className={th}>
                    <button onClick={() => setSortDir(d => d === "asc" ? "desc" : "asc")} className="inline-flex items-center gap-1.5 hover:text-white">
                      Next Due <ChevronsUpDown size={13} className="opacity-70" />
                    </button>
                  </th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={8} className="text-center py-16 text-gray-400 text-sm">
                    {records.length === 0
                      ? 'No maintenance plans yet. Click "New Plan" to schedule one.'
                  ...
                  <tr
                    ...
                    onClick={() => navigate(`/asset-management/maintenance-plan/${r.id}`)}
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-gray-800 leading-tight">{r.planName || "—"}</p>
                    </td>
                    <td className="px-5 py-3.5 text-gray-600">{r.assetName || "—"}</td>
                    <td className="px-5 py-3.5 text-gray-600">{r.maintenanceTypeName || "—"}</td>
                    <td className="px-5 py-3.5 text-gray-600">
                      {r.frequencyType === "Hour-Based" ? `Every ${r.hourTrigger || "—"} hrs` : (r.frequencyType ? `${r.frequencyType} (×${r.frequencyValue || "—"})` : "—")}
                    </td>
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => handleDelete(r.id, r.planName)}
