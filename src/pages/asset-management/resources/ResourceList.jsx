...
export default function ResourceList() {
  ...
  const handleDelete = async (id, resId) => {
    if (!window.confirm(`Delete resource allocation "${resId}"? This cannot be undone.`)) return;
    try {
      ...
      <div className="w-full space-y-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            Allocate human, machine and equipment resources against Work Orders
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative flex-1 min-w-[220px] max-w-sm">
            ...
            <input
              type="text"
              placeholder="Search by Resource ID, WO, supervisor…"
              value={search}
          ...
          <button
            onClick={() => navigate("/asset-management/resources/new")}
                ...
                <tr className="bg-gradient-to-r from-brand-800 to-brand-600">
                  <th className={th}>
                    <button onClick={() => setSortDir(d => d === "asc" ? "desc" : "asc")} className="inline-flex items-center gap-1.5 hover:text-white">
                      Resource ID <ChevronsUpDown size={13} className="opacity-70" />
                    </button>
                  </th>
                  <th className={th}>Work Order</th>
                  <th className={th}>Type</th>
                  <th className={th}>Assigned To</th>
                  <th className={th}>Planned Hrs</th>
                  <th className={th}>Actual Hrs</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr><td colSpan={8} className="text-center py-16 text-gray-400 text-sm">
                    {records.length === 0
                      ? 'No resource allocations yet. Click "New Resource" to get started.'
                  ...
                  <tr
                    ...
                    onClick={() => navigate(`/asset-management/resources/${r.id}`)}
                  >
                    ...
                    <td className="px-5 py-3.5 text-gray-600">{r.workOrderNo || "—"}</td>
                    <td className="px-5 py-3.5 text-gray-600">{r.resourceType || "—"}</td>
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-gray-800 leading-tight">{r.labourName || r.assetName || "—"}</p>
                    </td>
                    <td className="px-5 py-3.5 text-gray-600">{r.plannedHours !== "" && r.plannedHours != null ? r.plannedHours : "—"}</td>
                    <td className="px-5 py-3.5 text-gray-600">{r.actualHours !== "" && r.actualHours != null ? r.actualHours : "—"}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => handleDelete(r.id, r.resourceId)}
