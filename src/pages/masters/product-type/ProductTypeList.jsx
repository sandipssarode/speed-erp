...
export default function ProductTypeList() {
  ...
  const filtered = records.filter((r) => {
    ...
    return !q || r.typeId?.toLowerCase().includes(q) || r.name?.toLowerCase().includes(q);
  });
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Delete product type "${name}"? This cannot be undone.`)) return;
    try {
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-center py-16 text-gray-400 text-sm">
                      {records.length === 0 ? 'No product types yet. Click "Add New" to get started.' : "No records match your search."}
                    </td>
                  </tr>
                ) : (
                  paginated.map((r, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/masters/product-type/${r.id}`)} className="p-1.5 text-brand-500 hover:text-brand-600 hover:bg-brand-50 rounded" title="Edit">
                            <Edit2 size={13} />
                          </button>
