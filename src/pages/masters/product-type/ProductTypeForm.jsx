...
import { Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, Tag } from "lucide-react";
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {!isNew && form.name ? form.name.slice(0, 2).toUpperCase() : <Tag size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">{isNew ? "New Product Type" : (form.name || "Product Type")}</h1>
              <p className="text-sm text-white/70 mt-0.5">{isNew ? "Add a new product type to the master" : <span className="font-mono">{form.typeId}</span>}</p>
            </div>
          ...
          <div className="px-6 py-7 space-y-6">
            <Row label="Type ID">
              <input value={form.typeId} disabled placeholder="Auto-generated" className={inputCls(true)} />
            </Row>
            <Row label="Name" required error={errors.name}>
              <input value={form.name} onChange={e => setField("name", e.target.value)} disabled={isReadOnly} placeholder="e.g. Raw Material" className={inputCls(isReadOnly, errors.name)} />
            </Row>
            <Row label="Status">
              <label className="flex items-center gap-2.5 sm:pt-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-600">Deactivate (hides from new entries)</span>
              </label>
            </Row>
          </div>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save Product Type</button>
