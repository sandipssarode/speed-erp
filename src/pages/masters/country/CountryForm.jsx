...
import { Save, X, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft } from "lucide-react";

const inputCls = (disabled, error) =>
  `w-full px-3 py-2 text-sm border rounded-md transition-colors focus:outline-none
...
// Zoho-style left-aligned label row
function Row({ label, required, error, children, help }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1.5 sm:gap-5">
      <label className={`sm:w-44 sm:pt-2 text-sm shrink-0 ${required ? "text-gray-700 font-medium" : "text-gray-600"}`}>
        {label}{required && <span className="text-red-500 ml-0.5">*</span>}
      </label>
      <div className="flex-1 max-w-md">
        {children}
        {help && !error && <p className="text-xs text-gray-400 mt-1">{help}</p>}
        {error && <p className="text-xs text-red-500 mt-1 flex items-center gap-1"><AlertCircle size={11} className="shrink-0" />{error}</p>}
      </div>
    </div>
  );
}
  ...
  return (
    <Layout>
      <div className="w-full space-y-3">
        ...
        {toast && (
          <div className={`flex items-center gap-2 px-4 py-2.5 rounded-md text-sm border ${toast.type === "error" ? "bg-red-50 border-red-200 text-red-700" : "bg-green-50 border-green-200 text-green-700"}`}>
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-sm">
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between gap-3 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              {!isNew && (
                <button onClick={() => setShowChangelog(s => !s)} className={`flex items-center gap-1.5 text-xs px-3 py-1.5 border rounded-md font-medium transition-colors ${showChangelog ? "border-brand-200 bg-brand-50 text-brand-600" : "border-gray-300 text-gray-600 hover:bg-gray-50"}`}>
                  <FileText size={13} /> History
                </button>
              )}
              {mode === "view" && <button onClick={() => setMode("edit")} className="flex items-center gap-1.5 text-xs px-3 py-1.5 border border-brand-200 text-brand-600 hover:bg-brand-50 rounded-md font-medium"><Edit2 size={13} /> Edit</button>}
              {mode === "view" && !isNew && <button onClick={handleDelete} className="flex items-center gap-1.5 text-xs px-3 py-1.5 border border-red-200 text-red-500 hover:bg-red-50 rounded-md font-medium"><Trash2 size={13} /> Delete</button>}
            </div>
          </div>

          {showChangelog && !isNew && (
            <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/60">
              <h3 className="text-xs font-semibold text-gray-600 uppercase tracking-wide mb-2">History</h3>
          ...
          <div className="px-6 py-6 space-y-5">
            ...
            <Row label="Status">
              <label className="flex items-center gap-2 sm:pt-2 cursor-pointer">
