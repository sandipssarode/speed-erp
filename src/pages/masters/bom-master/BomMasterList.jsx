...
export default function BomMasterList() {
  ...
  const filtered = records.filter(r => {
    ...
    return !q ||
      r.bomId?.toLowerCase().includes(q) ||
      r.produceItemName?.toLowerCase().includes(q) ||
      r.produceItemCode?.toLowerCase().includes(q);
  });

  const pageCount = Math.max(1, Math.ceil(filtered.length / PAGE_SIZE));
  const paginated = filtered.slice((page - 1) * PAGE_SIZE, page * PAGE_SIZE);

  const handleDelete = async (id, bomId) => {
    if (!window.confirm(`Delete BOM "${bomId}"? This cannot be undone.`)) return;
    try {
      await api.del(`/api/bom-masters/${id}`);
  ...
  return (
    <Layout>
      <div className="w-full space-y-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">BOM Master</h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Bill of Materials — parent product, child items and quantity calculation
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={() => navigate("/masters/bom-master/new")}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-r from-brand-800 to-brand-600">
                  <th className={th}>BOM ID</th>
                  <th className={th}>Produce Item</th>
                  <th className={th}>Product Code</th>
                  <th className={th}>Qty to Produce</th>
                  <th className={th}>Child Items</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  <tr
                    ...
                    onClick={() => navigate(`/masters/bom-master/${r.id}`)}
                  >
                    ...
                    <td className="px-5 py-3.5">
                      <p className="font-medium text-gray-800 leading-tight">{r.produceItemName || "—"}</p>
                    </td>
                    <td className="px-5 py-3.5 font-mono text-xs text-gray-500">{r.produceItemCode || "—"}</td>
                    <td className="px-5 py-3.5 text-gray-600">{r.quantityToProduce ?? "—"}{r.unit ? ` ${r.unit}` : ""}</td>
                    <td className="px-5 py-3.5 text-gray-600">{r.lines?.length ?? 0}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => handleDelete(r.id, r.bomId)}
