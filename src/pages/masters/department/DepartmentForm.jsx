...
import { Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, Building2 } from "lucide-react";
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <button onClick={() => navigate("/masters/department")} className="flex items-center gap-1 text-sm text-gray-500 hover:text-brand-600 transition-colors font-medium">
          <ChevronLeft size={15} /> Department
        </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {!isNew && form.departmentName ? form.departmentName.slice(0, 2).toUpperCase() : <Building2 size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">{isNew ? "New Department" : (form.departmentName || "Department")}</h1>
              <p className="text-sm text-white/70 mt-0.5">{isNew ? "Add a new department to the master" : <span className="font-mono">{form.departmentCode}</span>}</p>
            </div>
          ...
          <div className="px-6 py-7 space-y-6">
            <Row label="Department Code">
              <input value={form.departmentCode} disabled placeholder="Auto-generated" className={inputCls(true)} />
            </Row>
            <Row label="Department Name" required error={errors.departmentName}>
              <input value={form.departmentName} onChange={e => setField("departmentName", e.target.value)} disabled={isReadOnly} placeholder="e.g. Purchase" className={inputCls(isReadOnly, errors.departmentName)} />
            </Row>
          </div>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save Department</button>
