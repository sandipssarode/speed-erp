...
import { Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, Boxes } from "lucide-react";
...
const emptyForm = () => ({
  assetTypeId:   `AT-${Date.now().toString().slice(-5)}`,
  assetTypeName: "",
  category:      "",
...
function validate(form, allRecords, editingId) {
  const e = {};
  if (!form.assetTypeId.trim())
    e.assetTypeId = "Asset Type ID is required.";
  else if (allRecords.some(r =>
    r.assetTypeId?.trim().toLowerCase() === form.assetTypeId.trim().toLowerCase() &&
    r.id !== editingId
  ))
    e.assetTypeId = "Asset Type ID already exists.";
  if (!form.assetTypeName.trim()) e.assetTypeName = "Asset Type Name is a required field.";
  if (!form.category) e.category = "Category is a required field.";
  return e;
}
...
export default function AssetTypeForm() {
  ...
  useEffect(() => {
    api.get("/api/asset-types").then(list => {
      setAllRecords(list);
      if (!isNew && id) {
        const found = list.find(r => r.id === id);
        if (found) setForm(found);
        else navigate("/masters/asset-type");
      }
    }).catch(console.error);
  }, [id, isNew]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        saved = await api.post("/api/asset-types", payload);
      } else {
        const payload = { ...form, updatedAt: now, updatedBy: userName, changelog: [...(form.changelog || []), changeEntry] };
        saved = await api.put(`/api/asset-types/${id}`, payload);
      }
      setForm(saved);
      setAllRecords(prev => isNew ? [...prev, saved] : prev.map(r => r.id === saved.id ? saved : r));
      setMode("view");
      setErrors({});
      showToast("Asset Type saved successfully.");
      if (isNew) navigate(`/masters/asset-type/${saved.id}`, { replace: true });
    } catch (err) {
  ...
  const handleDiscard = async () => {
    if (isNew) { navigate("/masters/asset-type"); return; }
    try {
      const found = await api.get(`/api/asset-types/${id}`);
      setForm(found);
    } catch { /* keep current */ }
    setMode("view");
    setErrors({});
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Delete asset type "${form.assetTypeName}"? This cannot be undone.`)) return;
    try {
      await api.del(`/api/asset-types/${id}`);
      navigate("/masters/asset-type");
    } catch (err) {
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <button onClick={() => navigate("/masters/asset-type")} className="flex items-center gap-1 text-sm text-gray-500 hover:text-brand-600 transition-colors font-medium">
          <ChevronLeft size={15} /> Asset Type
        </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {form.assetTypeId ? form.assetTypeId.slice(0, 2).toUpperCase() : <Boxes size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">{isNew ? "New Asset Type" : (form.assetTypeName || form.assetTypeId || "Asset Type")}</h1>
              <p className="text-sm text-white/70 mt-0.5">{isNew ? "Add a new asset type to the master" : <span className="font-mono">{form.assetTypeId}</span>}</p>
            </div>
          ...
          <div className="px-6 py-7 space-y-6">
            <Row label="Asset Type ID" required error={errors.assetTypeId} help={isNew ? "Auto-generated — cannot be changed later." : undefined}>
              <input
                ...
                maxLength={20}
                className={inputCls(isReadOnly || (!isNew), errors.assetTypeId)}
              />
            </Row>
            <Row label="Asset Type Name" required error={errors.assetTypeName}>
              <input
                value={form.assetTypeName ?? ""}
                onChange={e => setField("assetTypeName", e.target.value)}
                disabled={isReadOnly}
                placeholder="e.g. Electrical Equipment"
                className={inputCls(isReadOnly, errors.assetTypeName)}
              />
            </Row>
            <Row label="Category" required error={errors.category}>
              <select
                value={form.category ?? ""}
                onChange={e => setField("category", e.target.value)}
                disabled={isReadOnly}
                className={inputCls(isReadOnly, errors.category)}
              >
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save Asset Type</button>
