    ...
    <Layout>
      <div className="space-y-3 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <button onClick={() => navigate("/masters/asset-type")} className="hover:text-violet-500 transition-colors">Asset Type</button>
          {form.assetTypeId && <><ChevronRight size={12} /><span className="text-violet-600 font-medium">{form.assetTypeId}</span></>}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded px-4 py-2.5 flex items-center gap-2 flex-wrap shadow-sm">
          <button onClick={() => navigate("/masters/asset-type")}
        ...
        {showChangelog && (
          <div className="bg-white border border-gray-200 rounded p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              <FileText size={14} /> Audit Log — {form.assetTypeId || "New"}
            </h3>
        ...
        <div className="bg-white border border-gray-200 rounded shadow-sm">
          <div className="bg-gradient-to-r from-violet-900 to-violet-700 px-5 py-2.5 rounded-t flex items-center gap-4 text-white">
            <span className="font-bold text-base tracking-wide">{form.assetTypeId || "NEW ASSET TYPE"}</span>
            <span className="text-blue-200 text-sm">{form.assetTypeName || "—"}</span>
            {form.category && (
              <span className="ml-auto bg-white/10 text-white border border-white/20 px-2 py-0.5 rounded text-xs font-medium">
                {form.category}
              </span>
            )}
            {(mode === "new" || mode === "edit") && !form.category && (
          ...
          <div className="p-5">
            <div className="bg-gray-50 border border-gray-200 rounded p-4 space-y-4">
              <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Asset Type Details</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <Field label="Asset Type ID" required error={errors.assetTypeId}>
                  <TInput
                ...
                <Field label="Asset Type Name" required error={errors.assetTypeName}>
                  <TInput
