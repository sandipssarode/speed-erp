...
export default function VillageTalukaForm() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAllRecords(vList);
      setDistricts(dList.filter(d => !d.isDeactivated));
      if (!isNew && id) {
        const found = vList.find(r => r.id === id);
        if (found) setForm(found);
        else navigate("/masters/village-taluka");
      }
    }).catch(console.error);
  }, [id, isNew]);
  ...
  const handleDistrictChange = (districtId) => {
    ...
    if (errors.districtId) setErrors(prev => { const e = { ...prev }; delete e.districtId; return e; });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        saved = await api.put(`/api/village-talukas/${id}`, payload);
      }
      setForm(saved);
  ...
  const handleDiscard = async () => {
    if (isNew) { navigate("/masters/village-taluka"); return; }
    try {
      const found = await api.get(`/api/village-talukas/${id}`);
      setForm(found);
    } catch { /* keep current */ }
    setMode("view");
    setErrors({});
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Delete village/taluka "${form.villageName}"? This cannot be undone.`)) return;
    try {
      await api.del(`/api/village-talukas/${id}`);
      navigate("/masters/village-taluka");
    } catch (err) {
    ...
    <Layout>
      <div className="space-y-3 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          <button onClick={() => navigate("/masters/village-taluka")} className="hover:text-violet-500 transition-colors">Village / Taluka Master</button>
          {form.villageCode && <><ChevronRight size={12} /><span className="text-violet-600 font-medium">{form.villageCode}</span></>}
        </div>
