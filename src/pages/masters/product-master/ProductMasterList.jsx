...
export default function ProductMasterList() {
  ...
  const filtered = records.filter((r) => {
    ...
    const matchSearch = !q || r.productCode?.toLowerCase().includes(q) || r.productName?.toLowerCase().includes(q) || r.productTypeName?.toLowerCase().includes(q);
  ...
  const handleDelete = async (id, name) => {
    ...
    try {
      await api.del(`/api/product-masters/${id}`);
            ...
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="bg-blue-600">
                  ...
                  <th className="text-left px-4 py-2.5 text-xs font-semibold text-white uppercase tracking-wide">UOM</th>
                ...
                {paginated.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="text-center py-16 text-gray-400 text-sm">
                      {records.length === 0 ? 'No products yet. Click "Add New" to get started.' : "No records match your search."}
                    </td>
                  </tr>
                ) : (
                  paginated.map((r, i) => (
                    ...
                    >
                      <td className="px-4 py-2.5 font-mono text-xs font-semibold text-blue-600">{r.productCode}</td>
                      ...
                      <td className="px-4 py-2.5 text-gray-600 text-xs">{r.units || "—"}</td>
                      ...
                      <td className="px-4 py-2.5">
                        <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                          <button onClick={() => navigate(`/masters/product-master/${r.id}`)} className="p-1.5 text-blue-500 hover:text-blue-700 hover:bg-blue-50 rounded" title="Edit"><Edit2 size={13} /></button>
