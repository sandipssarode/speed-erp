...
import { Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, Map } from "lucide-react";
...
function validate(form, allRecords, editingId) {
  ...
  if (!form.stateCode.trim())
    ...
    r.stateCode?.trim().toLowerCase() === form.stateCode.trim().toLowerCase() &&
    r.id !== editingId
  ))
    e.stateCode = "State Code already exists.";
  if (!form.stateName.trim()) e.stateName = "State Name is required.";
  if (!form.countryId) e.countryId = "Country is required.";
  if (form.gstStateCode.trim() && !/^\d{2}$/.test(form.gstStateCode.trim()))
    e.gstStateCode = "GST State Code must be exactly 2 digits (e.g. 27).";
  return e;
}
...
export default function StateForm() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setAllRecords(sList);
      setCountries(cList.filter(c => !c.isDeactivated));
      if (!isNew && id) {
        const found = sList.find(r => r.id === id);
        if (found) setForm(found);
        else navigate("/system/states");
      }
    }).catch(console.error);
  }, [id, isNew]);
  ...
  const handleCountryChange = (countryId) => {
    ...
    if (errors.countryId) setErrors(prev => { const e = { ...prev }; delete e.countryId; return e; });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) navigate(`/system/states/${saved.id}`, { replace: true });
    } catch (err) {
  ...
  const handleDiscard = async () => {
    if (isNew) { navigate("/system/states"); return; }
    try {
      const found = await api.get(`/api/states/${id}`);
      setForm(found);
    } catch { /* keep current */ }
    setMode("view");
    setErrors({});
  ...
  const handleDelete = async () => {
    if (!window.confirm(`Delete state "${form.stateName}"? This cannot be undone.`)) return;
    try {
      await api.del(`/api/states/${id}`);
      navigate("/system/states");
    } catch (err) {
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {form.stateCode ? form.stateCode.slice(0, 2).toUpperCase() : <Map size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">{isNew ? "New State" : (form.stateName || "State")}</h1>
              <p className="text-sm text-white/70 mt-0.5">{isNew ? "Add a new state to the master" : <span className="font-mono">{form.stateCode}</span>}</p>
            </div>
          ...
          <div className="px-6 py-7 space-y-6">
            ...
            <Row label="State Name" required error={errors.stateName}>
              <input value={form.stateName} onChange={e => setField("stateName", e.target.value)} disabled={isReadOnly} placeholder="e.g. Maharashtra" className={inputCls(isReadOnly, errors.stateName)} />
            </Row>
            ...
            <Row label="GST State Code" error={errors.gstStateCode}>
              <input value={form.gstStateCode} onChange={e => setField("gstStateCode", e.target.value.replace(/\D/g, "").slice(0, 2))} disabled={isReadOnly} placeholder="e.g. 27" maxLength={2} className={inputCls(isReadOnly, errors.gstStateCode)} />
            </Row>
            <Row label="Status">
              <label className="flex items-center gap-2.5 sm:pt-2 cursor-pointer">
                ...
                <span className="text-sm text-gray-600">Mark this state as inactive</span>
              </label>
            </Row>
          </div>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save State</button>
