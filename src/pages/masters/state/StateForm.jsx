    ...
    <Layout>
      <div className="space-y-3 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          <button onClick={() => navigate("/system/states")} className="hover:text-violet-500 transition-colors">State Master</button>
          {form.stateCode && <><ChevronRight size={12} /><span className="text-violet-600 font-medium">{form.stateCode}</span></>}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded px-4 py-2.5 flex items-center gap-2 flex-wrap shadow-sm">
          <button onClick={() => navigate("/system/states")}
        ...
        {showChangelog && (
          <div className="bg-white border border-gray-200 rounded p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              <FileText size={14} /> Audit Log — {form.stateCode || "New"}
            </h3>
        ...
        <div className="bg-white border border-gray-200 rounded shadow-sm">
          <div className="bg-gradient-to-r from-violet-900 to-violet-700 px-5 py-2.5 rounded-t flex items-center gap-4 text-white">
            <span className="font-bold text-base tracking-wide">{form.stateCode || "NEW STATE"}</span>
            <span className="text-blue-200 text-sm">{form.stateName || "—"}</span>
            {form.countryName && (
              <span className="ml-auto bg-white/10 text-white border border-white/20 px-2 py-0.5 rounded text-xs font-medium">
                {form.countryName}
              </span>
            )}
            {(mode === "new" || mode === "edit") && !form.countryName && (
          ...
          <div className="p-5">
            <div className="bg-gray-50 border border-gray-200 rounded p-4 space-y-4">
              <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">State Details</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Field label="State Name" required error={errors.stateName}>
                  <TInput
                ...
                <Field label="Country" required error={errors.countryId}>
                  <TSelect
                    ...
                    disabled={isReadOnly}
                    options={countries.map(c => ({ value: c.id, label: `${c.countryCode} — ${c.countryName}` }))}
                    placeholder="Select Country"
                    error={errors.countryId}
                  />
                </Field>
                <Field label="GST State Code" error={errors.gstStateCode}>
                  <TInput
                    value={form.gstStateCode}
                    onChange={e => setField("gstStateCode", e.target.value.replace(/\D/g, "").slice(0, 2))}
                    disabled={isReadOnly}
                    placeholder="e.g. 27"
                    maxLength={2}
                ...
                <Field label="Deactivate">
                  <label className="flex items-center gap-2 mt-1 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-gray-600">Deactivate this state</span>
                  </label>
                </Field>
              </div>
            </div>
