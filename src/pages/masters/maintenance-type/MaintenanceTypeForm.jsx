    ...
    <Layout>
      <div className="space-y-3 max-w-3xl mx-auto">
        ...
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <button onClick={() => navigate("/masters/maintenance-type")} className="hover:text-brand-500 transition-colors">Maintenance Type</button>
          {form.typeId && <><ChevronRight size={12} /><span className="text-brand-600 font-medium">{form.typeId}</span></>}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded px-4 py-2.5 flex items-center gap-2 flex-wrap shadow-sm">
          <button onClick={() => navigate("/masters/maintenance-type")}
        ...
        {showChangelog && (
          <div className="bg-white border border-gray-200 rounded p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
              <FileText size={14} /> Audit Log — {form.typeId || "New"}
            </h3>
        ...
        <div className="bg-white border border-gray-200 rounded shadow-sm">
          <div className="bg-gradient-to-r from-brand-900 to-brand-600 px-5 py-2.5 rounded-t flex items-center gap-4 text-white">
            ...
            <div className="ml-auto flex items-center gap-2">
              {form.priority && (
                <span className={`px-2 py-0.5 rounded text-xs font-medium border ${PRIORITY_BADGE[form.priority] || ""}`}>
                  {form.priority}
                </span>
              )}
              {(mode === "new" || mode === "edit") && (
          ...
          <div className="p-5">
            <div className="bg-gray-50 border border-gray-200 rounded p-4 space-y-4">
              <p className="text-xs font-semibold text-gray-600 uppercase tracking-wide">Maintenance Type Details</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <Field label="Maintenance Name" required error={errors.maintenanceName}>
                  <TInput
                    value={form.maintenanceName}
                    onChange={e => setField("maintenanceName", e.target.value)}
                    disabled={isReadOnly}
                ...
                <Field label="Duration (hours)" error={errors.duration}>
                  <TInput
                    type="number"
                    value={form.duration}
