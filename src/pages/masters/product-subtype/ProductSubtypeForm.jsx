...
export default function ProductSubtypeForm() {
  ...
  const handleTypeChange = (typeId) => {
    ...
    setForm(prev => ({ ...prev, productTypeId: typeId, productTypeName: found?.name || "" }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isNew) {
        ...
        saved = await api.post("/api/product-subtypes", payload);
      } else {
        const payload = { ...form, updatedAt: now, updatedBy: userName, changelog: [...(form.changelog || []), changeEntry] };
        saved = await api.put(`/api/product-subtypes/${id}`, payload);
      }
      setForm(saved);
  ...
  const handleDiscard = async () => {
    if (isNew) { navigate("/masters/product-subtype"); return; }
    try { const found = await api.get(`/api/product-subtypes/${id}`); setForm(found); } catch { /* keep */ }
    setMode("view"); setErrors({});
  };

  const handleDelete = async () => {
    if (!window.confirm(`Delete sub-type "${form.subtypeName}"? This cannot be undone.`)) return;
    try { await api.del(`/api/product-subtypes/${id}`); navigate("/masters/product-subtype"); }
    ...
    <Layout>
      <div className="space-y-3 max-w-3xl mx-auto">
        <div className="flex items-center gap-1.5 text-xs text-gray-400">
          ...
          <button onClick={() => navigate("/masters/product-subtype")} className="hover:text-violet-500 transition-colors">Product Sub-type</button>
          {form.subtypeId && <><ChevronRight size={12} /><span className="text-violet-600 font-medium">{form.subtypeId}</span></>}
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded px-4 py-2.5 flex items-center gap-2 flex-wrap shadow-sm">
          <button onClick={() => navigate("/masters/product-subtype")} className="flex items-center gap-1.5 text-xs px-3 py-1.5 border border-gray-300 text-gray-600 hover:bg-gray-50 rounded font-medium"><ArrowLeft size={13} /> Back</button>
        ...
        {showChangelog && (
          <div className="bg-white border border-gray-200 rounded p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2"><FileText size={14} /> Audit Log — {form.subtypeId || "New"}</h3>
        ...
        <div className="bg-white border border-gray-200 rounded shadow-sm">
          <div className="bg-gradient-to-r from-violet-900 to-violet-700 px-5 py-2.5 rounded-t flex items-center gap-4 text-white">
            <span className="font-bold text-base tracking-wide">{form.subtypeId || "NEW SUB-TYPE"}</span>
            <span className="text-blue-200 text-sm">{form.subtypeName || "—"}</span>
            {form.productTypeName && <span className="ml-auto bg-white/10 text-white border border-white/20 px-2 py-0.5 rounded text-xs font-medium">{form.productTypeName}</span>}
          </div>
          <div className="p-5">
            <div className="bg-gray-50 border border-gray-200 rounded p-4 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <Field label="Sub-type Name" required error={errors.subtypeName} className="sm:col-span-2 lg:col-span-3">
                  <TInput value={form.subtypeName} onChange={e => setField("subtypeName", e.target.value)} disabled={isReadOnly} placeholder="e.g. Ferrous Metals" error={errors.subtypeName} />
                </Field>
              </div>
              <div className="flex items-center gap-2 mt-2">
                <input type="checkbox" id="isDeactivated" checked={!!form.isDeactivated} onChange={e => setField("isDeactivated", e.target.checked)} disabled={isReadOnly} className="rounded border-gray-300 text-violet-600 focus:ring-violet-400" />
