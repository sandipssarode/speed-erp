...
export default function UnitTypeList() {
  ...
  const filtered = records
    .filter(r => {
      ...
      const matchSearch =
        !q ||
        r.unitTypeId?.toLowerCase().includes(q) ||
        r.unitTypeName?.toLowerCase().includes(q);
      const matchStatus =
        statusFilter === "all" ||
    ...
    .sort((a, b) => {
      const r = (a.unitTypeName || "").localeCompare(b.unitTypeName || "");
  ...
  const handleDelete = async (id, name) => {
    if (!window.confirm(`Delete unit type "${name}"? This cannot be undone.`)) return;
    try {
      await api.del(`/api/unit-types/${id}`);
    ...
    <Layout>
      <div className="max-w-6xl mx-auto space-y-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            onClick={() => navigate("/masters/unit-type/new")}
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm min-w-[600px]">
              ...
              <tbody>
                {loading ? (
                  <tr><td colSpan={4} className="text-center py-16 text-gray-400 text-sm">Loading…</td></tr>
                ) : paginated.length === 0 ? (
                  <tr><td colSpan={4} className="text-center py-16 text-gray-400 text-sm">
                    {records.length === 0
                      ? 'No unit types yet. Click "New Unit Type" to add one.'
                  ...
                  <tr
                    ...
                    onClick={() => navigate(`/masters/unit-type/${r.id}`)}
                  >
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <span className="w-9 h-9 rounded-lg bg-brand-50 ring-1 ring-brand-100 text-brand-700 flex items-center justify-center text-xs font-bold shrink-0">
                          {(r.unitTypeName || "?").slice(0, 2).toUpperCase()}
                        </span>
                    ...
                    <td className="px-5 py-3.5 font-mono text-xs text-gray-400">{r.unitTypeId}</td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5">
                      <div className="flex items-center justify-end gap-1.5" onClick={e => e.stopPropagation()}>
                        ...
                        <button
                          onClick={() => handleDelete(r.id, r.unitTypeName)}
