...
import {
  Save, Trash2, Edit2, FileText, CheckCircle, AlertCircle, ChevronLeft, Layers,
} from "lucide-react";
...
function validate(form, allRecords, editingId) {
  const e = {};
  if (!form.unitTypeId.trim())
    e.unitTypeId = "Unit Type ID is required.";
  else if (allRecords.some(r =>
    r.unitTypeId?.trim().toLowerCase() === form.unitTypeId.trim().toLowerCase() && r.id !== editingId
  ))
    e.unitTypeId = "Unit Type ID already exists.";
  if (!form.unitTypeName.trim()) e.unitTypeName = "Unit Type Name is required.";
  return e;
}
...
export default function UnitTypeForm() {
  ...
  useEffect(() => {
    api.get("/api/unit-types").then(list => {
      ...
      if (!isNew && id) {
        ...
        else navigate("/masters/unit-type");
      }
    }).catch(console.error);
  }, [id, isNew]);
  ...
  const handleSave = async () => {
    ...
    try {
      let saved;
      if (isNew) {
        saved = await api.post("/api/unit-types", { ...form, id: Date.now().toString(), createdAt: now, updatedAt: now, createdBy: userName, updatedBy: userName, changelog: [entry] });
      } else {
        saved = await api.put(`/api/unit-types/${id}`, { ...form, updatedAt: now, updatedBy: userName, changelog: [...(form.changelog || []), entry] });
      }
      setForm(saved);
      setAllRecords(prev => isNew ? [...prev, saved] : prev.map(r => r.id === saved.id ? saved : r));
      setMode("view"); setErrors({});
      showToast("Unit Type saved successfully.");
      if (isNew) navigate(`/masters/unit-type/${saved.id}`, { replace: true });
    } catch (err) { showToast(err.message || "Failed to save.", "error"); }
  };

  const handleDiscard = async () => {
    if (isNew) { navigate("/masters/unit-type"); return; }
    try { const found = await api.get(`/api/unit-types/${id}`); setForm(found); } catch { /* keep */ }
    setMode("view"); setErrors({});
  };

  const handleDelete = async () => {
    if (!window.confirm(`Delete unit type "${form.unitTypeName}"? This cannot be undone.`)) return;
    try { await api.del(`/api/unit-types/${id}`); navigate("/masters/unit-type"); }
  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto space-y-4">
        <button onClick={() => navigate("/masters/unit-type")} className="flex items-center gap-1 text-sm text-gray-500 hover:text-brand-600 transition-colors font-medium">
          <ChevronLeft size={15} /> Unit Type Master
        </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {!isNew && form.unitTypeName ? form.unitTypeName.slice(0, 2).toUpperCase() : <Layers size={22} />}
            </div>
            <div className="min-w-0">
              <h1 className="text-xl font-bold text-white tracking-tight leading-tight">
                {isNew ? "New Unit Type" : (form.unitTypeName || "Unit Type")}
              </h1>
              <p className="text-sm text-white/70 mt-0.5">
                {isNew ? "Add a new unit category" : <span className="font-mono">{form.unitTypeId}</span>}
              </p>
            </div>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save Unit Type</button>
