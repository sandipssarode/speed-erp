  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto space-y-4">
        ...
        <button onClick={() => navigate("/masters/designation")} className="flex items-center gap-1 text-sm text-gray-500 hover:text-brand-600 transition-colors font-medium">
          <ChevronLeft size={15} /> Designation
        </button>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-brand-800 to-brand-600 px-6 py-5 flex items-center gap-4 flex-wrap">
            <div className="w-12 h-12 rounded-xl bg-white/15 ring-1 ring-white/20 flex items-center justify-center text-white font-bold shrink-0">
              {!isNew && form.designationName ? form.designationName.slice(0, 2).toUpperCase() : <IdCard size={22} />}
            </div>
          ...
          <div className="px-6 py-7 space-y-6">
            <Row label="Department Name" required error={errors.departmentName}>
              <select value={form.departmentName} onChange={e => setField("departmentName", e.target.value)} disabled={isReadOnly} className={inputCls(isReadOnly, errors.departmentName)}>
                <option value="">Select Department</option>
                {departments.map(d => <option key={d.departmentName} value={d.departmentName}>{d.departmentName}</option>)}
              </select>
            </Row>
            <Row label="Designation Code">
              <input value={form.designationCode} disabled placeholder="Auto-generated" className={inputCls(true)} />
            </Row>
            <Row label="Designation Name" required error={errors.designationName}>
              <input value={form.designationName} onChange={e => setField("designationName", e.target.value)} disabled={isReadOnly} placeholder="e.g. Purchase Manager" className={inputCls(isReadOnly, errors.designationName)} />
            </Row>
            <Row label="Level" required error={errors.level}>
              <select value={form.level} onChange={e => setField("level", e.target.value)} disabled={isReadOnly} className={inputCls(isReadOnly, errors.level)}>
          ...
          {editing && (
            <div className="px-6 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center gap-2.5">
              <button onClick={handleSave} className="flex items-center gap-2 text-sm px-6 py-2.5 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-700 hover:to-brand-600 text-white rounded-xl font-semibold shadow-md shadow-brand-200 transition-all"><Save size={15} /> Save Designation</button>
